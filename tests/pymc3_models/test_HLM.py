...
class HLMFitTestCase(HLMTestCase):
    def test_fit_returns_correct_model(self):
        ...
        print("")
        self.HLM.fit(self.X_train, self.cat_train, self.Y_train)

        self.assertEqual(self.num_cats, self.HLM.num_cats)
        self.assertEqual(self.num_pred, self.HLM.num_pred)
        #TODO: Think about whether this is the right testing tolerance
        np.testing.assert_almost_equal(self.alphas, self.HLM.v_params.means['alpha'], decimal=1)
        np.testing.assert_almost_equal(self.betas, self.HLM.v_params.means['beta'], decimal=1)
...
class HLMPredictProbaTestCase(HLMTestCase):
    def test_predict_proba_returns_probabilities(self):
        print("")
        self.HLM.fit(self.X_train, self.cat_train, self.Y_train)
        probs = self.HLM.predict_proba(self.X_test, self.cat_test)
        self.assertEqual(probs.shape, self.Y_test.shape)

    def test_predict_proba_raises_error_if_not_fit(self):
        with self.assertRaises(SonicScrewdriverError) as no_fit_error:
            HLM = SoothsayerHLM()
            HLM.predict_proba(self.X_train, self.cat_train)
...
class HLMPredictTestCase(HLMTestCase):
    def test_predict_returns_predictions(self):
        print("")
        self.HLM.fit(self.X_train, self.cat_train, self.Y_train)
        preds = self.HLM.predict(self.X_test, self.cat_test)
...
class HLMScoreTestCase(HLMTestCase):
    def test_score_scores(self):
        print("")
        self.HLM.fit(self.X_train, self.cat_train, self.Y_train)
        score = self.HLM.score(self.X_test, self.cat_test, self.Y_test)
...
class HLMSaveandLoadTestCase(HLMTestCase):
    def test_save_and_load_work_correctly(self):
        print("")
        self.HLM.fit(self.X_train, self.cat_train, self.Y_train)
        probs1 = self.HLM.predict_proba(self.X_test, self.cat_test)
        probs2 = self.HLM.predict_proba(self.X_test, self.cat_test)
        self.HLM.save(self.test_dir)

        HLM2 = HLM()

        HLM2.load(self.test_dir)

        self.assertEqual(self.HLM.num_cats, HLM2.num_cats)
        self.assertEqual(self.HLM.num_pred, HLM2.num_pred)
        self.assertEqual(summary(self.HLM.advi_trace), summary(HLM2.advi_trace))

        for key in self.HLM.v_params.means.keys():
            np.testing.assert_equal(self.HLM.v_params.means[key], HLM2.v_params.means[key])
